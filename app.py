import streamlit as st
import pandas as pd
import numpy as np
import time
import plotly.graph_objects as go

# --- 1. CONFIGURATION ---
st.set_page_config(page_title="AI Neural Deep Scanner v3", layout="wide")

# --- 2. THE DEEP NEURAL ENGINE (KOSORA LOGIC) ---
class DeepNeuralEngine:
    def __init__(self, game):
        self.game = game
        self.mem_key = f'mem_{game}'
        self.archive_key = f'archive_{game}'
        self.log_key = f'log_{game}'
        self.streak_key = f'streak_{game}'
        self.last_pred_key = f'last_p_{game}'
        
        # Initialize session states if not exists
        if self.mem_key not in st.session_state:
            st.session_state[self.mem_key] = [1.2, 2.8, 1.1, 3.5, 1.05]
        if self.archive_key not in st.session_state:
            st.session_state[self.archive_key] = [2.1, 1.05, 5.8, 1.1, 1.4, 2.0, 1.0, 3.2]
        if self.log_key not in st.session_state:
            st.session_state[self.log_key] = []
        if self.streak_key not in st.session_state:
            st.session_state[self.streak_key] = 0
        if self.last_pred_key not in st.session_state:
            st.session_state[self.last_pred_key] = None

    def analyze_deep_history(self):
        """Isesengura ry'amateka ya kera"""
        history = st.session_state[self.mem_key]
        archive = st.session_state[self.archive_key]
        if len(history) < 3: return 50
        
        last_three = history[-3:]
        matches = 0
        # Deep search in archive
        for i in range(len(archive) - 3):
            if archive[i:i+3] == last_three:
                matches += 1
        
        confidence = 60 + (matches * 12)
        return min(confidence, 99)

    def process_round(self):
        """Iyi niyo ikora byose: isuzuma Win/Loss, ikabika amateka, igatanga prediction nshya"""
        # 1. Simulate Actual Result
        actual_odd = round(np.random.uniform(1.0, 4.5), 2)
        
        # 2. Check Win/Loss from PREVIOUS prediction
        last_p = st.session_state[self.last_pred_key]
        if last_p:
            status = "WIN" if actual_odd >= last_p else "LOSS"
            if status == "WIN":
                st.session_state[self.streak_key] += 1
            else:
                st.session_state[self.streak_key] = 0
            
            # Log it
            st.session_state[self.log_key].insert(0, {
                "Time": time.strftime('%H:%M:%S'),
                "AI": f"{last_p}x",
                "Actual": f"{actual_odd}x",
                "Status": status
            })

        # 3. Update Memory
        st.session_state[self.mem_key].append(actual_odd)
        st.session_state[self.archive_key].append(actual_odd)
        if len(st.session_state[self.mem_key]) > 20: st.session_state[self.mem_key].pop(0)
        
        # 4. Generate NEW Prediction for next round
        conf = self.analyze_deep_history()
        if conf > 75:
            new_pred = round(np.random.uniform(1.6, 3.2), 2)
        else:
            new_pred = None # Wait mode
            
        st.session_state[self.last_pred_key] = new_pred
        return actual_odd, new_pred, conf

# --- 3. UI STYLING ---
st.markdown("""
    <style>
    .stApp { background-color: #050505; color: white; }
    .streak-badge {
        background: linear-gradient(90deg, #45ad15, #1d5a0a);
        padding: 10px; border-radius: 10px; font-weight: bold;
        text-align: center; border: 1px solid #45ad15; margin-bottom: 15px;
    }
    .prediction-card {
        background: #111; padding: 25px; border-radius: 15px;
        border: 2px solid #ff4b4b; text-align: center;
    }
    .status-text { font-family: 'Courier New', monospace; color: #45ad15; font-size: 12px; }
    </style>
    """, unsafe_allow_html=True)

# --- 4. LOGIN ---
if 'user' not in st.session_state:
    st.session_state['user'] = None

if st.session_state['user'] is None:
    st.markdown("<h2 style='text-align:center;'>DEEP NEURAL LOGIN</h2>", unsafe_allow_html=True)
    u = st.text_input("Agent ID")
    p = st.text_input("Access Key", type="password")
    if st.button("LOGIN", use_container_width=True):
        st.session_state['user'] = u
        st.rerun()
else:
    # --- 5. SIDEBAR ---
    with st.sidebar:
        st.markdown("### ⚙️ SYSTEM CONTROL")
        game_choice = st.radio("CHOOSE ENGINE", ["AVIATOR", "JETX"])
        st.write("---")
        st.write(f"🗄️ Archive: {len(st.session_state.get(f'archive_{game_choice}', []))} entries")
        if st.button("EXIT SYSTEM", use_container_width=True):
            st.session_state['user'] = None
            st.rerun()

    primary_color = "#ff4b4b" if game_choice == "AVIATOR" else "#ffcc00"
    engine = DeepNeuralEngine(game_choice)

    # --- 6. DASHBOARD (AUTONOMOUS) ---
    @st.fragment(run_every=6)
    def dashboard():
        # Process round and get data
        actual, next_pred, confidence = engine.process_round()
        
        st.markdown(f"<h2 style='text-align:center; color:{primary_color};'>{game_choice} DEEP SCANNER</h2>", unsafe_allow_html=True)

        # Winning Streak Badge
        streak = st.session_state[engine.streak_key]
        if streak >= 2:
            st.markdown(f"<div class='streak-badge'>🔥 WINNING STREAK: {streak} IN A ROW!</div>", unsafe_allow_html=True)

        # Prediction Display
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"""
                <div class='prediction-card' style='border-color:{primary_color};'>
                    <div style='color:#888;'>NEXT SIGNAL</div>
                    <div style='font-size:55px; font-weight:bold; color:{primary_color};'>{next_pred if next_pred else 'WAIT'}x</div>
                    <div class='status-text'>ARCHIVE MATCH: {confidence}%</div>
                </div>
            """, unsafe_allow_html=True)
            
        with c2:
            st.markdown(f"""
                <div class='prediction-card' style='border-color:#444;'>
                    <div style='color:#888;'>ACTUAL RESULT</div>
                    <div style='font-size:55px; font-weight:bold; color:white;'>{actual}x</div>
                    <div class='status-text'>LIVE SYNCED...</div>
                </div>
            """, unsafe_allow_html=True)

        # Graph
        history = st.session_state[engine.mem_key]
        fig = go.Figure(go.Scatter(x=list(range(len(history))), y=history, mode='lines+markers', line=dict(color=primary_color, width=4)))
        fig.update_layout(paper_bgcolor='black', plot_bgcolor='black', font_color='white', height=180, margin=dict(l=0,r=0,t=10,b=0), xaxis=dict(visible=False))
        st.plotly_chart(fig, use_container_width=True)

        # Logs
        st.write("### 📊 ENGINE PERFORMANCE")
        for log in st.session_state[engine.log_key][:5]:
            l_color = "#45ad15" if log['Status'] == "WIN" else "#ff4b4b"
            st.markdown(f"""
                <div style="background:#111; padding:10px; border-radius:10px; margin-bottom:5px; border-left: 5px solid {l_color};">
                    AI: <b>{log['AI']}</b> | Result: <b>{log['Actual']}</b> | <span style="color:{l_color}; font-weight:bold;">{log['Status']}</span>
                </div>
            """, unsafe_allow_html=True)

    dashboard()
